...
### Phase 6 Tier-A cluster-implementation ADRs (ADRs 0097-0098, 0102; first three of the cluster set)
...
| [0102](0102-tier-a-cluster-4-node-schema-redesign.md) | Tier-A Cluster 4: Node schema redesign (14 new columns + 10-value `node_type` TEXT[] enum + 3 validator checks + `'unclassified'` transitional default per schema-only landing) | Accepted |
...
>
> Session S-0213 lands Cluster 4 (ADR 0102 + migration `0140_nodes_schema_redesign.sql` + paired rollback). Commits 14 new columns to `public.nodes` (no existing-column changes; no row-level UPDATEs — schema-only per Adjudication 1) including `node_type TEXT[] NOT NULL DEFAULT '{unclassified}'` with 10-value element CHECK (`threshold_concept`, `bridge_concept`, `disciplinary_practice`, `text_excerpt`, `historical_context`, `misconception`, `comparative_lens`, `assessment_task`, `subfield`, `unclassified` transitional); `tradition_label TEXT[]` open vocabulary; 5 JSONB columns (`canonical_sources`, `approved_examples`, `misconceptions`, `assessment_items`, `mastery_evidence`); 7 scalar additions (`disciplinary_domain`, `granularity`, `audience_tags`, `accessibility_notes`, `assumed_background`, `jargon_load`, `cultural_specificity`). Three validator checks: `node_type_unclassified` (severity mode-switched at module constant), `node_threshold_concept_lacks_assessment_items` (soft-warn), `node_lacks_cultural_specificity` (soft-warn). Round-trip verified end-to-end. Re-verifies ADR 0094 T1-A + ADR 0095 T1-A at landing; partially closes ADR 0094 T1-B (D1, D2, D3, D4, D8 settled within Tier-A scope; full closure depends on D6 + D7 at Cluster 5). Per-domain `node_type` backfill defers to Cluster 4 sequel sessions (likely 4-6 sessions, one per subdomain, claiming migration slots 0141-0149).

> ADRs 0099–0101 sit in the numerical range between Cluster 2 and Cluster 4 but file under `engine/adr/` (session-close friction mitigations, Engine-inspired hook installation, subprocess isolation for graph_audit); see [`engine/adr/README.md`](../../engine/adr/README.md) for the engine collection.
...
5. Add an `Added` line to the current build session's per-session changelog entry at `engine/changelog/<YYYY>/S-NNNN-<topic>.md` (per [ADR 0092](../../engine/adr/0092-per-session-changelog-directory.md) — supersedes the pre-S-0198 monolithic ENGINE_LOG.md flow).
