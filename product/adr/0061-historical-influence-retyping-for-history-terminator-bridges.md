...
**S-0208 amendment — Cluster 2 retyping** (per [ADR 0098](0098-tier-a-cluster-2-edge-type-taxonomy-and-three-relation-layering.md)). The 17 `historical_influence` edges this ADR introduced now carry `edge_layer = 'historical_influence'` + `edge_type = 'influenced_by'` post-migration 0130. The retyping is lossless — ADR 0061's commitment (historical_influence as a distinct predicate class from pedagogical_prerequisite) is preserved within Cluster 2's three-layer structure: the `edge_layer` value retains the "historical_influence" name verbatim, and `influenced_by` is the most general of the three historical sub-types (`influenced_by` / `received_via` / `reacted_against`) per ADR 0098 premise 3. A future historical-influence audit (per ADR 0098 T2-A readiness criterion) may retype specific edges among the 17 to `received_via` or `reacted_against` where the framing fits more precisely; the audit informs ADR 0098 amendment. This ADR's `Status: Accepted` is unchanged.
