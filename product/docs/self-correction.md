...
The concrete retrieval shape is settled in [ADR 0087](../adr/0087-two-hop-neighborhood-retrieval-shape.md): one-hop immediate prerequisites; two-hop traversal walks `pedagogical_prerequisite` + `historical_influence` edges (`historical_influence` is recognition-visible during teaching even though it remains display-only for the Discovery surface per [ADR 0061](../adr/0061-historical-influence-retyping-for-history-terminator-bridges.md), and mastery computation still walks `pedagogical_prerequisite` only — the asymmetry is deliberate); alias resolution returns all candidates above a similarity threshold with the highest-confidence one tagged `presumed_match`; no named per-turn token budget — the pedagogical-degradation discipline absorbed into [ADR 0014](../adr/0014-sonnet-teaches-opus-reviews.md) governs downshift.
