...
It ships as an **open-source personal project** (Apache 2.0). The official Apple App Store binary ships free-download-with-IAP-unlock under the maintainer's personal Apple Developer account; users supply their own Anthropic API key, stored on-device in iOS Keychain (BYOK). The builder maintains it because the pedagogy is worth maintaining, calibrated for first-encounter learners because the builder values the pedagogical discipline of that calibration (per [ADR 0012](../adr/0012-freshman-defaults-autodidact-ceiling.md)) — not because of market-fit. The option to grow into an institutional product, a non-profit, a grant-funded organization, or an acquisition target is foreclosed *as a refusal, not a deferral* per [ADR 0065](../adr/0065-oss-pivot-and-byok-disposition.md), including in the success case. The freshman-defaults calibration survives because it protects against **builder-bias drift** — the failure mode where knowing how the system works obscures whether it's usable cold to a learner encountering it for the first time; the calibration now extends to the in-app BYOK key-onboarding flow as well as the teaching surface. The success criterion is "an app I would pay for if it weren't mine," verified at Phase 9 by a small TestFlight cohort cold-test that also exercises the BYOK onboarding flow (see [`business.md`](business.md) Personal Project Disposition).
...
2. **Operating discipline must not corrupt pedagogy.** Under the OSS+BYOK disposition the threat shape is *user*-funnel-mechanic — engineering retention nudges that bait users into more API spend on their own dime — alongside the *builder*-funnel-mechanic concerns the prior framing identified. The same discipline answers both: free download with one-time IAP unlock and no subscription, no institutional regime, no funnel-mechanic surfaces, static polish only. A structural free-tier of the app (graph browse, sample syllabus) is permitted but must not be engineered as a conversion funnel. ([ADR 0065](../adr/0065-oss-pivot-and-byok-disposition.md) — supersedes [ADR 0029](../adr/0029-personal-financial-cost-ceiling.md) and [ADR 0035](../adr/0035-multi-platform-apple-expansion.md); chain preserved through [ADR 0032](../adr/0032-personal-project-disposition.md) which already carries `Superseded by ADR 0035`.)
