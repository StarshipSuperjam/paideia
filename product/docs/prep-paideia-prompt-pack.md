...
- ~~Session Lifecycle & Interaction Design~~ (2026-04-07): Concept engagement as atomic learning unit, mode transition signals (textual only, no timing inference), proficiency as implied transition, organic mastery-verification framing per [ADR 0013](../adr/0013-mastery-verification-organic-escalation.md), five-syllabus hard cap, routing after concept completion, cross-syllabus convergence surfacing. Decisions recorded in session-lifecycle.md. *(S-0016 note: the prior home-screen-as-spatial-surface + reward-visual-system framing originally settled here was superseded at S-0013/S-0014 by [ADR 0033](../adr/0033-mission-realignment-structured-guidance-for-self-learners.md) and [ADR 0034](../adr/0034-discovery-planning-engagement-triad.md); session-lifecycle.md was substantially rewritten at S-0014 against the Discovery / Planning / Engagement triad. The home-screen-vs-syllabus tension previously logged in tensions.md is marked Resolved at S-0016.)*
...
- ~~Seed Graph & Node Schema~~ (2026-04-09): Node schema settled (id, label, domain TEXT[], summary, teaching_notes, aliases, rigor_score_computed, rigor_score_adjustment, provenance, confidence, status, superseded_by, graph_version_added, timestamps). Rigor score computation formula (α·inbound_rigor_mass + β·prerequisite_count + γ·downstream_depth, V1 weights 0.5/0.3/0.2). Two-column override model (adjustments don't propagate). Domain tags as flat labels (no hierarchy; concept clustering from community detection on edge topology per [ADR 0018](../adr/0018-flat-domain-tags-community-detection.md), with the consumer at S-0014 shifted from spatial layout to Discovery-surface catalog organization per [ADR 0034](../adr/0034-discovery-planning-engagement-triad.md)). Louvain/Leiden community-detection algorithm preserved as graph-analysis primitive. OWL/RDF rejected. Seed graph rebuild reclassified as implementation task (direct database seeding, not JSON). architecture.md split into five files. philosophy-graph-seed.json archived. Decisions recorded in architecture.md and ui-architecture.md.
...
## ~~Session 13: Gamification & Milestone Tone~~ — Closed at S-0016

*Topic: Drawing the line between emotionally resonant milestones and gamification*

> **Closed 2026-04-30 (S-0013 / S-0014 / S-0016).** This session prompt is preserved as historical context only. The gamification tone tension was closed in two stages: [ADR 0033](../adr/0033-mission-realignment-structured-guidance-for-self-learners.md) (S-0013) retired the prior reward-visual-system framing the prompt was authored against; [ADR 0034](../adr/0034-discovery-planning-engagement-triad.md) (S-0014) committed the Discovery / Planning / Engagement triad with no separate reward / library / accumulation surface; the [ADR 0027](../adr/0027-rendering-policy-prompt-layer-contract.md) S-0016 revision adds the corresponding voice-discipline closure (forbidden-token enumeration extended with reward / trophy / glow / mastery-visualization language and globe / spatial-traversal metaphors). The settled answer to the prompt's *Resolution* question: completion is information, not spectacle; the AI's tone is recognition not congratulation per [ADR 0013](../adr/0013-mastery-verification-organic-escalation.md); badges / streaks / leaderboards / XP / levels are categorically foreclosed; reward-layer configurability for younger learners is foreclosed at the per-app level (per [ADR 0032](../adr/0032-personal-project-disposition.md) commitment 6 — no dynamic-feature surfaces). The tension entry in [`docs/tensions.md`](tensions.md) is marked Resolved at S-0016 in this same session. The original prompt body is preserved in git history at S-0015 (commit 356c870) and earlier; it is not reproduced here because its body referenced now-forbidden tokens whose enumeration is the artifact this session would have authored.
