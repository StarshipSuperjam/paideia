...
**Cluster 4 additions (S-0213, ADR 0102).** As of S-0213 per [ADR 0102](../adr/0102-tier-a-cluster-4-node-schema-redesign.md), the nodes table carries 14 additional columns supporting the Phase 6 Tier-A node-schema redesign (synthesis.md §Cluster 4 lines 141-176). Migration [`0140_nodes_schema_redesign.sql`](../seed-graph/migrations/0140_nodes_schema_redesign.sql) adds them as schema-only — all 380 existing rows carry column defaults; per-domain backfill defers to Cluster 4 sequel sessions claiming migration slots 0141-0149.

- **node_type** (TEXT[] NOT NULL DEFAULT `'{unclassified}'`): Multi-valued enum per ADR 0102 Adjudication 2 (kant_walkthrough.md §3.2.1.A's multi-typing reality — e.g., `phenomenology` fits 3 enum values cleanly). Per-element CHECK on 10-value vocabulary (`threshold_concept`, `bridge_concept`, `disciplinary_practice`, `text_excerpt`, `historical_context`, `misconception`, `comparative_lens`, `assessment_task`, `subfield`, `unclassified` transitional). The `unclassified` default fires the `node_type_unclassified` validator soft-warn (severity mode-switched at module constant `NODE_TYPE_UNCLASSIFIED_FIRE_MODE`; escalates to hard-fail at the LAST per-domain backfill sequel session, which also drops `unclassified` from the enum). See [`PREDICATE_MANIFEST.md`](../seed-graph/migrations/PREDICATE_MANIFEST.md) "Node-side predicates: node_type vocabulary" for per-value semantics. `is_threshold_concept` does NOT exist as a separate boolean per ADR 0102 Adjudication 2 — the predicate is `'threshold_concept' = ANY(node_type)`.
- **disciplinary_domain** (TEXT, nullable): Discipline level (`philosophy`, `literary_theory`, `history`, etc.). **Coexists with existing `domain TEXT[]`** at sub-discipline / topic-area level per ADR 0102 Adjudication 7 — the two fields operate at different semantic granularities and both persist. Future ADRs may commit a discipline taxonomy via CHECK constraint; not constrained at landing.
- **granularity** (TEXT, nullable, CHECK `IN ('coarse', 'medium', 'fine')`): Per synthesis line 154. Informs syllabus generation about whether to expand the node into sub-nodes (coarse) or treat as atomic (fine).
- **audience_tags** (TEXT[] NOT NULL DEFAULT `'{}'`): Open vocabulary (no element CHECK); intended values include `intro`, `intermediate`, `advanced`, `majors`, `non-majors`, `multilingual_cohort`.
- **canonical_sources** (JSONB NOT NULL DEFAULT `'[]'::jsonb`): Element shape `{author, year, title, identifier?}`. One of five embedding-quality-affecting fields per `adversarial_review.md` E.12.2.
- **approved_examples** (JSONB NOT NULL DEFAULT `'[]'::jsonb`): Element shape `{description, source_ref?}`. Curated examples the teaching LLM may use; distinct from free-form examples in `teaching_notes` because structurally retrievable.
- **misconceptions** (JSONB NOT NULL DEFAULT `'[]'::jsonb`): Element shape `{description, remediation_ref?, remediation_note?}`. **Lightweight encoding** — the full misconception sub-graph (separate `misconception` nodes + `common_misconception_about` / `unlearning_required_before` edges) lands in Cluster 5 per synthesis §Cluster 5 and [ADR 0094](../adr/0094-phase-6-scope.md) dependency order C2 + C4 → C5.
- **assessment_items** (JSONB NOT NULL DEFAULT `'[]'::jsonb`): Element shape `{type, prompt, expected_response_form, rationale}`. **JSONB rather than FK** per ADR 0102 Adjudication 8 — the assessments table doesn't exist; JSONB is forward-compatible. A future Phase 7+ cluster can extract to an FK relation when assessment authoring becomes load-bearing. The `node_threshold_concept_lacks_assessment_items` validator soft-warn fires on threshold-concept-typed nodes with empty `assessment_items` per paper_1:L136.
- **mastery_evidence** (JSONB NOT NULL DEFAULT `'[]'::jsonb`): Element shape `{evidence_kind, observation_target}`. What counts as evidence of mastery for this node; consumed by the Phase 7+ mastery-computation pipeline per [ADR 0085](../adr/0085-server-side-mastery-computation-confirmed.md).
- **accessibility_notes** (TEXT, nullable): Free-text notes for accessibility considerations.
- **assumed_background** (TEXT[] NOT NULL DEFAULT `'{}'`): Background assumptions the node makes that aren't captured by inbound `hard_prerequisite` / `soft_prerequisite` edges. Useful for syllabus pre-flight checks.
- **jargon_load** (TEXT, nullable, CHECK `IN ('low', 'medium', 'high')`): Per synthesis line 162.
- **cultural_specificity** (TEXT, nullable): Records the cultural background the node assumes (e.g., `'Western_continental_philosophy'`). The `node_lacks_cultural_specificity` validator soft-warn fires on NULL values (equity-metadata gate per synthesis line 171).
- **tradition_label** (TEXT[] NOT NULL DEFAULT `'{}'`): Multi-valued open vocabulary per ADR 0102 Adjudication 6. NO element CHECK because the proposed Cluster 8 enum was found under-specified per kant_walkthrough §3.5.D (cross-traditional, sub-traditional, and discipline-as-tradition cases all need accommodation). **Per ADR 0102 Adjudication 3 (strict ADR 0008 read)**, movement-shaped entities like `phenomenology`-the-20th-century-movement route via `tradition_label`, NOT via `node_type=['historical_context']`. Cluster 8 (Phase 7+ deployment governance) wires the strip-at-LLM-boundary mechanism; Cluster 4 owns the column.
...
- **timestamps**: created_at and updated_at are table stakes.
- **edge_type**: TEXT field. **As of S-0208 per [ADR 0098](../adr/0098-tier-a-cluster-2-edge-type-taxonomy-and-three-relation-layering.md), `edge_type` is constrained to the per-layer controlled vocabulary** in the compound CHECK on `(edge_layer, edge_type)` — 16 valid pairs across the three layers (`pedagogical_dependence` / `conceptual_relatedness` / `historical_influence`). Pre-S-0208 framing was "intentionally unconstrained" so new types could be added without migration; Cluster 2 supersedes that — extension is now `ALTER TABLE ... DROP CONSTRAINT ... ADD CONSTRAINT` per ADR 0098 premise 4. The pedagogical-layer sub-types (`hard_prerequisite`, `soft_prerequisite`, `helpful_bridge`, etc.) carry structural pedagogical meaning consumed by traversal, syllabus generation, and mastery computation; the historical-layer sub-types (`influenced_by`, `received_via`, `reacted_against`) are display features per the original framing; the conceptual-layer sub-types (`affinity_with`, `contrasts_with`, `same_problem_family`) are reserved-but-unused at the migration 0130 landing — Cluster 5 (misconception sub-graph) is the natural first author. The canonical predicate registry lives at [`product/seed-graph/migrations/PREDICATE_MANIFEST.md`](../seed-graph/migrations/PREDICATE_MANIFEST.md).
- **edge_layer**: TEXT NOT NULL field added at S-0208 per [ADR 0098](../adr/0098-tier-a-cluster-2-edge-type-taxonomy-and-three-relation-layering.md). One of three values (`pedagogical_dependence` / `conceptual_relatedness` / `historical_influence`) enforced by the `edges_edge_layer_valid` CHECK constraint. The layer determines which `edge_type` values are valid via the coupling CHECK above. Teaching-layer queries asking for "prerequisites of X" must filter to `edge_layer = 'pedagogical_dependence'`; cross-layer traversal must be explicit (the `edge_cross_layer_default_routing` validator soft-warn is committed in ADR 0098 Consequences as a Phase 7 routing-layer deliverable).
