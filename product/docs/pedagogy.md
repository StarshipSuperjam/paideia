...
The system can proactively introduce a concept into the learner's awareness before they ask for it. This happens when a prerequisite concept is about to become relevant on the learning path, or when mastery of a current concept makes a cross-domain bridge newly reachable (per the bridge-surfacing-in-context convention in [ADR 0034](../adr/0034-discovery-planning-engagement-triad.md)). The AI does not wait to be asked — it surfaces the concept, gives a vivid framing that makes the learner want to engage, and ends with an open question the domain grapples with. This sets the concept to "exposed" in the learner model. It is the threshold act before teaching begins.
...
The system never names this mechanic to the user. The user experiences it as the AI asking a harder question than usual. If they succeed, the concept's state quietly advances to mastered (which the Planning surface reflects per [ADR 0034](../adr/0034-discovery-planning-engagement-triad.md)) and any cross-domain bridges that have just become reachable surface contextually in the next exchange. If they don't, the AI continues teaching — no failure state is announced.
...
After a successful verification, the concept's state advances to mastered. The Planning surface reflects this in every syllabus that includes the concept (per [ADR 0034](../adr/0034-discovery-planning-engagement-triad.md)); cross-domain bridges that have just become reachable surface contextually in subsequent exchanges, per the bridge-surfacing-in-context convention. The AI's tone is recognition, not congratulation — it treats the user as someone who accomplished something real, not someone who needs a gold star. A brief note of what mastery of this concept opens up (new connections, neighboring concepts now tractable) is the substantive consequence. There is no reward affordance, no glow, no badge: completion is information, not spectacle.
