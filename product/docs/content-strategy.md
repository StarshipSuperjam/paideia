...
Node-level onward-reading pointers to SEP are committed in [ADR 0088](../adr/0088-sep-chunk-resolver-index.md): a `sep_chunks(node_id, sep_url, section_anchor, section_title, position)` junction table with one-to-many cardinality. Section anchors and titles are nullable, so a node can carry an article-level pointer (graceful degradation) when section-anchor precision isn't yet identified. The teaching layer queries by `node_id` and surfaces the top-ordered section in onward-reading recommendations. URLs and section anchors are facts about SEP's structure (uncopyrightable per Feist), consistent with the structural-reference-not-content-source posture above.
