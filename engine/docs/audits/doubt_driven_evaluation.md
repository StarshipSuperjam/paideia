...
## Workflow application 5 — S-0206 path proposal for the third validator_runtime_phase_regression fire

### CLAIM

Apply Path A (algorithmic optimization) to `validate_timestamp_helper_bypass` via a byte-level keyword pre-filter that skips `ast.parse()` for any `.py` file whose contents do not contain `"isoformat"`, `"strftime"`, or `"fromisoformat"`. Expected to drop the function's runtime from ~330ms (23% of structural phase) to ~30ms by skipping 38 of 41 walked files (only 3 actually contain the keyword); structural-phase pipeline median expected to drop from ~1071ms to ~770ms — at the 750ms threshold edge but reliably under for the rolling-3 window if `validate_adr_back_reference_orphan`'s sub-second variance stays within current range. If post-fix verification still shows sustained breach, layer Path C (modest target adjustment) on top.

### EXTRACT

1. The dominant cost in `validate_timestamp_helper_bypass` is `ast.parse()` + `ast.walk()` for each walked file, not the `read_text()` I/O. Per-file AST construction across 21,458 LOC of Python source is CPU-bound.
2. Of the 41 walked `.py` files (excluding `test_*.py` per the function's existing scope), only 3 actually contain `.isoformat` / `.strftime` / `.fromisoformat` callsites: `probe_push_gate.py`, `scan_dependabot_prs.py`, and `validate.py` itself.
3. A byte-level `in` test against the source text before `ast.parse()` is a sound pre-filter — any file containing the keyword falls through to the existing AST path, so the optimization cannot produce false negatives.
4. The other 38 files (which contain neither the helper-routed timestamp emissions nor any ad-hoc emissions) require no inspection — they have nothing for the AST walk to find.
5. The optimization is independent of other structural-phase work — no cross-function coupling, no file-cache sharing with `_tracked_md_files`, no contract change.
6. The dominant cost in the structural phase overall is `validate_adr_back_reference_orphan` at ~629ms (most of `validate_repo_structure`'s 1100ms). That function's S-0151 inverted-index is still applied; the cost is intrinsic to reading ~211 non-ADR `.md` files. No Path A headroom for that function within this session's scope.
7. After Path A, the structural-phase pipeline median should land at ~770ms. The 1.5× threshold is 750ms. The breach-detection rule requires sustained breach across 3 consecutive runs, so a 770ms median with sub-second variance can drift above/below the threshold per-run without firing as long as the median across the window stays under.
8. If post-Path-A measurement shows sustained breach, the honest secondary fix is Path C (target adjustment): bump `VALIDATOR_PHASE_TARGETS_MS["duration_structural_ms"]` to ~700ms (1.4× of measured ~770ms steady-state) and fold ADR 0063 Consequences per ADR 0036.

### DOUBT

1. Premise 1 — accept. Profile data verifies: `validate_timestamp_helper_bypass` median 330ms, 23% of structural sum. The function reads → ast.parse → ast.walk; the `read_text` portion is fast on macOS APFS (a 4665-line file like `validate.py` reads in single-digit ms); the AST parse + walk is the residual.
2. Premise 2 — verified empirically this session (`grep -l "isoformat\|strftime\|fromisoformat" engine/tools/*.py` returns 3 non-test, non-allowlist files: `probe_push_gate.py`, `scan_dependabot_prs.py`, `validate.py`).
3. Premise 3 — **what about a future file that uses the helper-routed timestamp emission in a way that touches the keyword but is correct?** *Concrete scenario:* a file imports `timestamps.emit` and aliases it as `iso = timestamps.emit`; the file contains the *function name* `iso` but not the *attribute name* `isoformat`. Result: pre-filter correctly skips (no keyword), AST path correctly skips (no `.isoformat` attribute access). *Counter-scenario:* a file contains a docstring or a string literal mentioning `isoformat` (e.g., this very ADR-adjacent doc file's example code blocks). Result: pre-filter sees the keyword, falls through to AST path, AST path finds zero `.isoformat` *Call* nodes, returns zero soft-warns — semantically identical to status quo. The pre-filter strictly broadens what's parsed, never narrows it relative to the substring test.
4. Premise 4 — **what about a file that splits the keyword across lines?** *Concrete scenario:* `obj.iso\\\nformat()` — Python's lexer treats line continuations specially, but the byte text contains `iso\\\nformat`, not the substring `isoformat`. The pre-filter would skip; the AST walk would have found a `Call` node with `func.attr == "isoformat"`. *Mitigation:* Python source style virtually never splits identifiers across `\` line-continuations (ruff and ruff format both forbid this in idiomatic code; the project's pre-commit hook gates on both). Risk is non-zero but extremely low; the project's codebase contains no such splits today (verified by the empirical `grep -l` on the current corpus). If the project later adopts a style that allows this, the regression check will fire on the next sustained breach — but the situation is fundamentally about coding style, not the pre-filter logic.
5. Premise 5 — accept. The optimization is a localized change to one function; no shared state, no contract surface.
6. Premise 6 — accept. The cost-attribution profile (clean-cache n=5) shows `validate_adr_back_reference_orphan` at 629ms (80% of cascade subtotal of 790ms), `validate_adr_consequences_deliverable_audit` at 109ms, `validate_superseded_adr_currency` at 51ms, `validate_duplicate_adr_number` at 1ms. The biggest item is intrinsic to file I/O; the S-0151 inverted-index already covers the algorithmic improvement available to that function.
7. Premise 7 — **what if the post-Path-A measurement is consistently ~800ms with variance into the 850-900ms range?** *Concrete scenario:* `validate_adr_back_reference_orphan` is I/O-bound; its variance under FS cache pressure could be 200-500ms (today's harness saw 607-2324ms in the cascade harness's bad-conditions run). The 750ms threshold could be breached on 3 consecutive runs under heavier I/O load. *Mitigation:* re-measure under realistic conditions post-fix; if breach is reproducible across 5 runs, layer Path C on top with evidence. The combined approach is acknowledged in the implementation plan; this is not a single-path commitment.
8. Premise 8 — **what if Path C is the right call entirely (skip Path A)?** *Concrete scenario:* if the corpus growth has organically raised steady-state and there's no single algorithmically-improvable hot path, target adjustment is the honest move. *Refutation:* the pre-filter on `validate_timestamp_helper_bypass` is a genuine algorithmic improvement (90% file skip with no semantic change), not a hack. Pure Path C without applying the available Path A would mean accepting CPU waste — sub-optimal engineering. The ordering — Path A first, Path C as fall-back — is defensible.

### RECONCILE

- Premise 1, 2, 3, 5, 6 — accept.
- Premise 4 — accept with named risk (line-continued identifier split is theoretically possible but not present in current corpus; if introduced, the regression check would surface the missed cost — same backstop logic the S-0151 fix relied on). Add a one-line comment on the pre-filter explaining the substring-skip semantics so a future reader understands the assumption.
- Premise 7 — **revise the implementation plan**: Path A first, then verify with ≥5 sequential default-mode runs; if median stays sub-750ms across the window, T1-B closure is Path A and T1-C is N/A. If sustained breach reproduces, additionally apply Path C (target bump to ~700ms) and fold ADR 0063 Consequences per ADR 0036. Document both outcomes in the readiness-note third-fire close section.
- Premise 8 — accept; ordering is correct.

**Net assessment.** The Path A proposal survives the doubt pass. Two refinements added to the implementation plan: (a) inline comment on the pre-filter explaining substring-skip semantics; (b) post-fix verification sequence with explicit fall-back to Path C if sustained breach reproduces.

**Counterfactual.** Had the doubt pass surfaced that the pre-filter risks false negatives in the current corpus (e.g., line-continued split identifiers actually present), the proposal would have been revised to use a more robust pre-filter (tokenize + token-stream scan, or a per-function memoized AST cache keyed on file mtime). The empirical `grep -l` check on the current 41-file corpus refuted that scenario.
