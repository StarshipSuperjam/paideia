...
| ID | Finding | Status |
|---|---|---|
| T1-A | Anonymization invariant must catch publication-name surfaces from a real SEP fetch | **Pending the first interactive exercise.** Synthetic adversarial fixtures pass at S-0106; the load-bearing test is fetching one real SEP entry (e.g., `https://plato.stanford.edu/entries/proposition/`) and confirming the brief comes through clean. The interactive audit-closeout session that fortifies S-0104 medium-confidence verdicts is that exercise. |
| T1-B | Robots.txt fetch and parse against the real SEP host must succeed under default User-Agent | **Pending the first interactive exercise.** Synthetic robots.txt routing passes; SEP's actual robots.txt may have shape the synthetic does not exercise. Fallback: robots-unavailable proceeds conservatively, so a robots-fetch failure is non-blocking. |
| T1-C | Rate-limit (default 2.0s) must be empirically observable against real network | **Pending the first interactive exercise.** Synthetic timer mocking confirms the algorithm; real-network observation confirms wall-clock delay actually fires. |
| T1-D | One-fetch round-trip against a real SEP URL must produce a non-empty `FocusingBrief.entries` | **Pending the first interactive exercise.** Synthetic SEP-shaped HTML produces entries; the real SEP HTML structure may have evolved since the parser fixtures at `engine/tools/test_fixtures/sep/` were captured. If extraction yields zero entries, fall back to: fetch a fixture page from the cached fixtures, compare structural shape, file Issue with the diff. |
...
Once recorded, this report's T1-A through T1-D rows update from "Pending" to "Resolved at S-XXXX" with the closing session's id.
