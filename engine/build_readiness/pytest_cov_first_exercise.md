...
- **T1-A — pending closeout at S-0136 post-push.** After the deliverable commits land on `main` and parent FF's, observe GitHub Actions run on the post-S-0136 SHA. The `Run pytest engine/tools with coverage` step should exit 0 (current coverage 80% ≥ floor 78%). Closes when the run completes green.
