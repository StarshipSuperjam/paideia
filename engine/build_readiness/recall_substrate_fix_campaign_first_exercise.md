...
| ID | Criterion | Status |
|---|---|---|
| **T1-A** | Empirical re-verification completes (ADR 0090 commitment 1). Each of the three S-0184 modes has a documented probe + observed result against current mempalace 3.3.5 + the project's actual palace. Verification report lands at `engine/docs/audits/mempalace_state_S-0186.md` (or equivalent). | **CLOSED (S-0186).** All three modes confirmed broken at 22K and re-verified at 673-drawer scale. Mode 2 confirmed broken even at 673 drawers, falsifying the upstream "scale-bound at 328K" claim. Report at [`engine/docs/audits/mempalace_state_S-0186.md`](../docs/audits/mempalace_state_S-0186.md). |
| **T1-B** | ADR 0079 recurrence-interval investigation completes (ADR 0090 commitment 4). Persist-condition probe, threshold-channel verification, write-path distribution between drawers/closets collections — all documented. Findings inform the rebuild-cadence decision under commitment 2a. May produce an ADR 0079 amendment or follow-up ADR. | **CLOSED (S-0186).** Root cause: `link_lists.bin = 0 bytes` after S-0145 rebuild because chromadb's `_persist()` never fired with the mempalace `_HNSW_BLOAT_GUARD` 50_000 threshold inherited at collection creation. The post-rebuild threshold switch to 100 was retroactive-only and did not flush the un-flushed initial-rebuild state. Fix verified: rebuild with `sync_threshold=3` at creation time forces per-add persist; raised to 100 post-rebuild. ADR 0079 amendment recommended for S-0187 to bake this into [`mempalace_rebuild_hnsw.py`](../tools/mempalace_rebuild_hnsw.py). |
| **T1-C** | First confirmed-broken-mode mitigation wired successfully (ADR 0090 commitment 2). Specifically: if T1-A surfaces Mode 1 as broken at our scale, the scheduled rebuild wiring lands in session-shutdown-sequence (or whichever cadence the investigation recommends). If Mode 3 broken, one-time prune executes. If Mode 2 broken, ADR 0057 element 4 in-body amendment lands. At least one mitigation must successfully execute end-to-end. | **PARTIAL CLOSE (S-0186).** Two mitigations wired: (a) one-shot prune via new `prune_mempalace.py --audit-sessions-pollution` mode drained 21,893 drawers + existing `--audit-orphan-wings` drained 110 drawers; (b) defensive post-hook prune via new [`mempalace_post_hook_prune.py`](../tools/mempalace_post_hook_prune.py) wired into `mempalace-hook-wrapper.sh`. The rebuild-tool amendment (`sync_threshold=3` at creation) is forward to S-0187. The Mode 2 ADR 0057 element 4 amendment is NOT needed (Mode 2 verified working post-rebuild). |
| **T1-D** | Upstream coordination process documented (ADR 0090 commitment 3). `engine/operations/mempalace-operations.md` gains a "Upstream coordination" subsection naming the watch-list (Issues #1082 / #1398 / #1489; PRs #1463 / #1511 / #1452 / #1428) and the retire-workaround pattern. | **CLOSED (S-0186).** "Pollution control" subsection authored in `mempalace-operations.md` under "Maintenance probes" naming the two-layer mitigation, the upstream PR #1511 retirement criterion, and the watch-and-retire pattern. The full watch-list (Issues #1082 / #1398 / #1489; PRs #1463 / #1511 / #1452 / #1428) is named in ADR 0090 commitment 3 and remains the operational reference. |
