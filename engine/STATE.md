...
| **Last build session** | **S-0144 (2026-05-12) — MemPalace HNSW rebuild (Issue #112 closed) + flush-primitive root-cause investigation (Issue #113 filed).** User pushback at boot ("this is the second time I've had to do this rebuild. Why? Can it be prevented?") widened scope from "run the rebuild" to "rebuild + ≤30 min upstream investigation." **Rebuild:** executed via upstream `mempalace repair --mode from-sqlite --archive-existing --yes` (NOT the project's `engine/tools/mempalace_rebuild_hnsw.py` — its S-0084 safety check refuses on `onnx_mini_lm_l6_v2` which is now mempalace v3.3.5's default; upstream's `--mode from-sqlite` is the post-S-0084 canonical path). 16,775 records rebuilt (16,724 drawers + 51 closets) against scratch palace, verified via cosine 0.841 on `ADR 0044 skill conversion` known-content query, atomically swapped to live. Post-swap MCP `mempalace_search` returns vector results (`matched_via: drawer`, not BM25 fallback) — vector search operational. Three backups retained: `~/.mempalace/palace.S-0144-pre-rebuild` (pre-rebuild live snapshot), `~/.mempalace/palace.S-0144-pre-swap` (live just-before-swap, includes 61 in-session auto-capture drawers absent from rebuilt palace — fully reconstructible from session JSONL), `~/.mempalace/palace.S-0144-scratch.pre-rebuild-20260512-170331` (upstream's archive of scratch pre-rebuild). **Flush-primitive investigation:** chromadb's `PersistentLocalHnswSegment._apply_batch` only calls `_persist()` when `_num_log_records_since_last_persist >= _sync_threshold` (`chromadb/segment/impl/vector/local_persistent_hnsw.py:280-281`); default `sync_threshold = 1000` (`hnsw_params.py:80`); `_persist()` writes the `index_metadata.pickle` whose absence triggers the "never flushed metadata" state. `client.close()` does NOT call `_persist()` (only `close_persistent_index()` which closes file handles). For mempalace's 1–3-drawers-per-session write pattern, the 1000 threshold is rarely reached between session boundaries → process exit drops in-memory metadata. **Prevention lever identified but deferred:** `hnsw:sync_threshold` collection metadata (configurable per-collection) can be lowered (e.g., to 5–10) to force per-write persist. Mempalace creates collections without this metadata (`mempalace/mcp_server.py:419-422`, `repair.py:196-210`) — closing the gap requires upstream patch OR project-side `collection.modify()` wrapper. ADR-scale (needs threshold-value deliberation + upstream-vs-project-side authoring choice), filed as [Issue #113](https://github.com/StarshipSuperjam/paideia/issues/113). Validator: 0 hard-fails; baseline soft-warns only (`issue_collision` 15 / `missing_rigor_score` 360 / `orphan_leaf` 1) — `mempalace_hnsw_divergence` cleared. 2 commits: eager-claim ca52d31 + close (pending). |


## Next session work item

**S-0145 — pick next eligible unit.** Phase 6 entry remains blocked behind OQ-DEC1-A/B/C/D tension settlement (4 tensions decide-before Phase 6, 16+ sessions stale; see [`product/docs/tensions.md`](../product/docs/tensions.md)). Either tackle the OQ-DEC1 deliberation set OR pick one of the ready-to-execute alternatives below.
...
- **[Issue #113](https://github.com/StarshipSuperjam/paideia/issues/113)** — HNSW sync_threshold prevention (filed at S-0144). ADR-scale: threshold-value deliberation + upstream-vs-project-side authoring choice + wrapper tool + first-exercise readiness note. Closes the recurring HNSW-metadata-not-flushed root cause documented in S-0144's investigation.
...
Plan file (S-0144 approved): [`~/.claude/plans/glistening-forging-shannon.md`](~/.claude/plans/glistening-forging-shannon.md).
