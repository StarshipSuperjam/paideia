...
- **Two surfaces per recipe.** The Skill body and the ops doc both describe the procedure. Drift between them is a new failure mode. Mitigation: the ops doc remains the source-of-truth (Layer 1 under ADR 0036), and the Skill body is generated by adaptation, not by independent authoring; updates to the procedure flow doc → Skill, not Skill → doc. The next health check (S-0060 under the corrected cadence per [ADR 0043](0043-hook-architecture.md)) audits drift.
