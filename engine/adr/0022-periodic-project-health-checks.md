...
The project runs **periodic health checks** at a configurable cadence (default every 10 sessions as of S-0033 — was every 30 from S-0001 to S-0032; tightened by user direction at S-0032 because the S-0032 MemPalace audit surfaced enough silent failures, see Consequences amendment below). The cadence trigger fires automatically at session boot when `next_id mod health_check_cadence == 0` — the slot about to be claimed is the cadence-numbered session; the user accepts (the session's work is the audit, per [`engine/operations/health-check.md`](../operations/health-check.md)) or defers (proceed with planned work; trigger fires again next time the modulus is satisfied).
