...
The `github-actions` ecosystem reads `.github/workflows/validate.yml` (the single CI workflow per [ADR 0083 (engine)](0083-validate-py-mirror-to-ci.md)). When an action version pin (`astral-sh/setup-uv@v3`, `actions/checkout@v4`, etc.) has a minor/patch release available, Dependabot opens a PR. Action pins are typically major-version-locked; minor/patch updates are usually backward-compatible.
...
- **Cost — false-positive risk on transitive bumps.** A minor bump in a transitive that breaks our import-site is possible (rare, but non-zero). Mitigation: CI (`validate.py` + bandit + tests via [ADR 0083 (engine)](0083-validate-py-mirror-to-ci.md)) runs on every Dependabot PR. A PR that breaks CI is held for review; the reviewer adjudicates whether to pin around it (`>=X.Y.Z,<X.Y+1`) or skip the bump.
...
**First-PR-arrival verification (S-0147):** Dependabot opened 11 PRs (#94–#105) on 2026-05-12, ahead of the expected Monday-cadence first run. All 11 were major-version bumps (chromadb 0.5→1.5.9; mypy 1.10→2.1.0; pytest 8→9.0.3; pypdfium2 4→5.8.0; ruff 0.4→0.15.12; psycopg 3.1→3.3.4; reportlab 4.0→4.5.1; bandit 1.7→1.9.4; beautifulsoup4 4.12→4.14.3; setup-uv 3→7; actions/checkout 4→6). The `pip-minor-and-patch` grouping correctly did NOT bundle them per decision 3 — major-version bumps open individually. None of the 11 were minor/patch (because the floor pins in `pyproject.toml` were authored with intentionally generous floors per [ADR 0064](0064-uv-lockfile-and-reproducible-builds.md) — Dependabot's first scan caught up to current-resolution-state in a single batch of majors). Each PR's commit message form is `chore(deps): bump <pkg> from <a> to <b>`. CI triggered per [ADR 0083 (engine)](0083-validate-py-mirror-to-ci.md) on each PR.

The first-PR-arrival exercise also surfaced a gap not anticipated at S-0133: the boot surface (per [ADR 0048](0048-handoff-narrowing-and-github-issues-for-cross-session-deferrals.md)) counts Issues, not PRs. Eleven PRs sat invisible to every session boot between S-0133 and S-0147. The S-0147 session authored [ADR 0080](0080-boot-time-dependency-version-visibility.md) to close the visibility gap (boot-time Dependabot PR surface + validator soft-warn `dependabot_pr_stale`); per ADR 0080's first-exercise readiness note at [`engine/build_readiness/dependabot_visibility_first_exercise.md`](../build_readiness/dependabot_visibility_first_exercise.md), the same S-0147 session processes all 11 PRs as the mechanism's first natural exercise. The pre-S-0147 "First-PR-arrival verification" empirical surface was effectively a posture-only commitment; ADR 0080 is the mechanism that closes the posture.
...
- [ADR 0083 (engine)](0083-validate-py-mirror-to-ci.md) — CI mirror (Dependabot PRs are validated by the same workflow as human PRs).
...
- [ADR 0080](0080-boot-time-dependency-version-visibility.md) — S-0147 boot-surface mechanism that closes the "First-PR-arrival verification" empirical gap this ADR's record amended.
- [`engine/operations/dependency-discipline.md`](../operations/dependency-discipline.md) — Layer 1 ops doc; security-update workflow section updated in the same session to past-tense citing this ADR; "Stale Dependabot PR handling" subsection added at S-0147 per [ADR 0080](0080-boot-time-dependency-version-visibility.md).
