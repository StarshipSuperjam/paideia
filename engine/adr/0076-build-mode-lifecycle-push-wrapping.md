...
### Per-worktree liveness marker — closing the pre-eager-claim bystander gap

The two close-side defenses above protect the *caller's own* worktree and apply the conservative reap criteria to everything else. Neither covers a **bystander live worktree**: a sibling session that is alive but is not the sweep's caller. The boot-time bulk sweep is gated on `CONFLICT_STATUS == "no-conflict"`, and `check_session_conflict.py` derives that status from `register_state.json` `current_status`. A session in plan/exploration mode has **not run the eager-claim ritual**, so `current_status` is still `closed` — the conflict check reports no conflict, the sweep gate opens, and the sibling's sweep reaps the plan-mode worktree. A pre-eager-claim session has made no commits, so its worktree is `claude/*` + clean + merged into `main` — it matches every reap criterion. This is the S-0142 failure mode (a session's working folder wiped from under it) recurring through an uncovered path; it hit S-0155 directly.

The fix is a per-worktree liveness marker. [`engine/tools/hooks/session-start.sh`](../tools/hooks/session-start.sh) writes a `session-live` marker into each worktree's private git dir (`.git/worktrees/<name>/session-live` for a linked worktree) at **every** session boot — crucially, *before* the eager-claim ritual, so a plan/exploration session that never claims a slot still carries the marker. The marker lives in the private git dir, outside the working tree, so it never appears in `git status` and cannot make a worktree look dirty. Both [`engine/tools/sweep_worktrees.sh`](../tools/sweep_worktrees.sh) and [`engine/tools/routine_worktree_sweep.py`](../tools/routine_worktree_sweep.py) carry a pre-flight that resolves each candidate worktree's private git dir and preserves the worktree when the marker's mtime is within a 24h freshness window. The window matches `check_session_conflict.py`'s stale-session threshold: a genuinely abandoned worktree's marker goes stale after 24h and the worktree becomes reapable again, so the project does not leak worktrees. The residual edge — a session running longer than 24h loses marker protection — is bounded by the same threshold the conflict check already uses, and is strictly safer than the pre-S-0157 state (zero protection for bystanders).

The marker write is best-effort: `session-start.sh` logs a fail line and proceeds if the git dir cannot be resolved or the write fails. The marker is a preserve *hint*, never a blocker — its absence simply leaves the existing conservative reap criteria in force.

ADR 0053 trigger evaluation: no new mechanism class — both sweep tools and the SessionStart hook already existed; this is a defense-in-depth refinement of the existing worktree-sweep mechanism, the same character as the S-0143 skip-caller pre-flight. Criterion 1 (new session mode) NO; criterion 2 (new validator soft-warn) NO; criterion 3 (new state file the *boot procedure* reads) NO — the marker is a sweep-time preserve hint, not a boot-procedure input. Criterion 4 (≥5 tooling files OR ≥3 ops docs): 3 production tooling files (`session-start.sh`, `sweep_worktrees.sh`, `routine_worktree_sweep.py`) + 1 new test file (`test_sweep_worktrees.py`) + 1 extended test file + 2 ops docs (`session-shutdown-sequence.md`, `cross-references.md`) + this subsection — evaluates NO on the same reading as the S-0143 subsection. No first-exercise readiness note required: the preserve path is covered by 13 dedicated pytests (8 in `test_routine_worktree_sweep.py`, 5 in the new `test_sweep_worktrees.py`) against tmp-dir worktree fixtures, and the natural in-the-wild exercise is simply the next sibling boot preserving a live worktree.

Cascade landings: [`engine/operations/session-shutdown-sequence.md`](../operations/session-shutdown-sequence.md) step 11 gains the liveness-marker preservation paragraph; [`engine/operations/cross-references.md`](../operations/cross-references.md) gains a `session-start.sh` row and updates the two sweep-tool rows; [`CLAUDE.md`](../../CLAUDE.md) "Posture vs machinery" updates the post-close-sweep bullet to name the liveness-marker defense. Discovered in-band at S-0155 ([Issue #120](https://github.com/StarshipSuperjam/paideia/issues/120)); closed at S-0157.
