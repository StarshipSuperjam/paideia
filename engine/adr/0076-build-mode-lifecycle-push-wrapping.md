...
### Post-close worktree sweep adopts routine-mode parity

Build-mode close invokes [`engine/tools/routine_worktree_sweep.py`](../tools/routine_worktree_sweep.py) after the close push, matching the leave-no-mess posture routine close has carried since [ADR 0054 Amendment](0054-lifecycle-push-wrapping-against-default-branch-push-gate.md). The sweep tool is mode-agnostic in mechanism — its pre-flight checks mirror `sweep_worktrees.sh` exactly per the tool's own docstring (lines 36-38: `claude/*` branch, working tree clean, branch merged into main). The "routine_" prefix in the tool's name reflects its first consumer, not a mode binding. Build mode reuses as-is; no sibling tool is authored (which would mirror the lifecycle-push sibling pattern of decision 1 but isn't warranted — the lifecycle-push sibling exists because routine + build deliverable verifiers differ, whereas the sweep verifier is identical). Rename to `worktree_sweep_at_close.py` is queued as cosmetic cleanup; the current name's first-consumer prefix is documented here as the canonical disposition.

[`engine/operations/session-shutdown-sequence.md`](../operations/session-shutdown-sequence.md) step 11 is the Layer 1 source-of-truth for the invocation; the `session-shutdown-sequence` Skill body mirrors per [ADR 0044](0044-skill-conversion-recipe-vs-reference.md) doc → skill direction. The sweep runs from inside the soon-to-be-removed worktree; `routine_worktree_sweep.py` chdir's to parent first so the process working directory is not the worktree being removed.

[`engine/tools/sweep_worktrees.sh`](../tools/sweep_worktrees.sh) (the bulk-cleanup utility, sibling to the per-close sweep) carries a skip-caller's-worktree safety check so the utility is safe to invoke mid-session. Detection: `git rev-parse --show-toplevel` from `$PWD` before chdir to the repo root, compared against each iterated worktree path during the sweep loop, skip on match. Without this check, a build session whose eager-claim push had merged its branch into `main` (the lifecycle-push wrapper's default behavior) would have its own worktree targeted alongside the other stale worktrees — the dirty-tree pre-flight check is not sufficient because clean post-eager-claim states are common.

ADR 0053 trigger evaluation: no new mechanism — `routine_worktree_sweep.py` already exists and was exercised at routine closes since S-0072. Build-mode invocation is a wiring change. The bulk-tool skip-current check is new mechanism but bounded to one shell script's safety logic with one integration point; criterion 4 (≥5 tooling files OR ≥3 ops docs) evaluates NO. No first-exercise readiness note required.

Cascade landings: [`engine/operations/cross-references.md`](../operations/cross-references.md) gains a row for `session-shutdown-sequence.md` ↔ `routine_worktree_sweep.py` and a Tools section entry for the sweep tool. [`CLAUDE.md`](../../CLAUDE.md) "Posture vs machinery" gains a bullet covering both build-mode lifecycle-push wrapping (which had no entry pre-S-0142) and the post-close sweep wiring.
