...
This adds a fourth layer to ADR 0052's three-layer routine-boot defense (freshness gate / lock / loser-recovery), aligned to the same "mechanically-verified bounded shape" posture: refuse pushes outside the verified shape; perform the push only when the shape matches the contract for that mode.
...
- **Cross-machine concurrency.** Same residual as ADR 0052 — first-push-wins. The wrapper is per-checkout state.
