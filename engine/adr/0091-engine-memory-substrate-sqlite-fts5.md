...
- `engine/memory/verify_recall.py` + `engine/docs/audits/engine_memory_recall_S-0191.md` — harness runs 10 hand-engineered queries against a seeded synthetic-fixture corpus (mechanical scope per user-adjudicated reframing at S-0191 boot). The pre-S-0191 spec referenced an `outcome_summary.boot_search_results` archive field — this field does not exist (the `mempalace_zero_citations_after_search` soft-warn stored counts only, not query text). The S-0191 verify_recall doc records the correction; the cite-worthy gate slid to S-0192 close per the same reframing.
- **Cutover gate (S-0191 mechanical scope; cite-worthy gate slides to S-0192).** Synthetic-corpus PASS at S-0191 closes T1-C mechanically; real-data PASS at S-0192 close (via `verify_recall.py --real-data` against the post-migration substrate) closes the cite-worthy comparison. `verification_passed: true` lands on Issue #138 at S-0192 close.
