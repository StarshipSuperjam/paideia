...
- **S-0041 amendment: strict-modulo replaced by overdue-catchup.** The strict-modulo logic this ADR committed (`next_id mod cadence == 0`) was correct at the natural-cadence slot but introduced a silent-slide failure mode when the cadence-aligned slot was consumed by user-directed work — the trigger silently slipped forward by a full cadence. S-0040's slot was claimed for the deferred MemPalace wing-filter fix (per S-0040's `outcome_summary`), and the next natural firing slipped to S-0050 (a 19-session gap, nearly 2x the configured 10-session cadence). S-0041 replaced strict-modulo with overdue-catchup tracking: `register_state.json` gains a `last_audit_session` field, `engine/tools/health_check.py` bumps it at report-emit time, and the SessionStart hook computes `slots_since = next_id - last_audit_session` (firing when `slots_since >= cadence` with separate "due" and "overdue" surface text). `engine/tools/validate.py` gained a `health_check_overdue` soft-warn so a silently-failing hook cannot mask the slide. Carrier updates landed in S-0041 across this Consequences section, [ADR 0022](0022-periodic-project-health-checks.md) Decision + Consequences, `engine/operations/health-check.md`, `engine/operations/session-build-lifecycle.md` and its skill, `engine/operations/tools-validate-interpretation.md`, and `.claude/commands/start-engine.md`. The legacy strict-modulo path is preserved in `session-start.sh` as a fallback when `last_audit_session` is absent (legacy `register_state.json`); the fallback emits a `last-audit-absent-falling-back-to-strict-modulo` log line so the regression is visible.
