...
1. **Two-layer decision recording** — every settled decision lands in BOTH an ADR AND a MemPalace `decision`-tagged drawer. The S-0030 health-check audit measured this: 1 decision drawer for 42 ADRs across 442 drawers in the `paideia` wing. The discipline was carried in name only.
...
- **Soft enforcement leaves drift possible.** A hook that reminds but does not block can be ignored. The S-0030 audit measured posture's actual yield (1/42 = 2.4% compliance); soft enforcement is expected to lift this materially but not to 100%. The next health check (cadence at S-0060 under the corrected logic) will measure.
