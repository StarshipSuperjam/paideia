...
- New: [`engine/tools/scrub_env.py`](../tools/scrub_env.py), [`engine/tools/scrub_env.sh`](../tools/scrub_env.sh), engine/tools/probe_palace.py (retired at S-0193 per [ADR 0091](0091-engine-memory-substrate-sqlite-fts5.md) — chromadb probe surface migrated to `engine.memory.connection.healthcheck()`), [`engine/tools/probe_repo.py`](../tools/probe_repo.py), [`engine/tools/conftest.py`](../tools/conftest.py), [`engine/tools/test_scrub_env.py`](../tools/test_scrub_env.py), [`engine/tools/test_probe_palace.py`](../tools/test_probe_palace.py), [`engine/tools/test_probe_repo.py`](../tools/test_probe_repo.py).
...
**HNSW divergence detection + non-destructive restoration.** `probe_palace.py` consumes upstream's read-only `mempalace repair-status` to compute HNSW-vs-SQLite divergence; the probe emits a `[probe-palace] divergence: HNSW=N1 SQLite=N2 (P%)` stderr line. Divergence ≥ 10% promotes the probe's exit code from 0 to 1; `validate.py`'s `validate_shared_state_health()` parses the line and emits the `mempalace_hnsw_divergence` soft-warn, with a LOUD-attention body at ≥ 30% that includes a destructive-repair carve-out warning. Restoration uses [`engine/tools/mempalace_rebuild_hnsw.py`](../tools/mempalace_rebuild_hnsw.py) — a direct chromadb rebuild that bypasses `mempalace repair --mode legacy` (which destroys SQLite embedding rows; see `engine/operations/mempalace-operations.md` "Known issues"). The tool reads `(embedding_id, document, metadata)` tuples directly from `chroma.sqlite3` for every drawer in the METADATA segment, deletes the collection preserving its `metadata` (e.g., `hnsw:space: cosine`), recreates, and re-adds via `collection.add(ids, documents, metadatas)` without explicit embeddings — chromadb re-computes via the registered default embedding function and writes both SQLite metadata segment and HNSW vector segment in sync. SQLite-direct extraction sidesteps the failure mode where `get(include=["embeddings"])` raises `InternalError: Error finding id` on divergent rows. Trade-off: rebuilt vectors are not byte-identical to originals (the default embedding function may have shifted), but the alternative is to lose the divergent rows' data entirely. Operational discipline: always run against a scratch palace copy first; atomic-rename swap to live gated on post-rebuild `repair-status < 1%` divergence. The rebuild tool is project-internal; it duplicates capability that ought to live in upstream's `mempalace repair` (tracked at [MemPalace/mempalace#1394](https://github.com/MemPalace/mempalace/issues/1394)). If upstream lands a non-destructive mode, `mempalace_rebuild_hnsw.py` becomes vestigial and should retire.
...
- [ADR 0091](0091-engine-memory-substrate-sqlite-fts5.md) — S-0188 substrate replacement. The mempalace-shaped instantiation of this ADR's atomic-snapshot-rollback discipline retires with the substrate at S-0193 (`probe_palace.py` deletes; SQLite WAL is the new durability story). The broader shared-state-integrity discipline survives: `scrub_env.sh` + boot-time health probes continue to apply to all subprocesses; the new substrate's `engine/memory/connection.py` follows the same scrub + probe pattern.
