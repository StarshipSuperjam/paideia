...
**Audit verdict fortification via inline evidence annotation.** Inline `evidence` text on a `pedagogical_prerequisite` (or `historical_influence`) edge is the necessary-and-sufficient signal that the edge was deliberately reasoned about in audit context. Proximity-against-migration-tuples (matching on `(source_id, target_id)` against migration VALUES lists without inspecting the row's `evidence` column) exhibit false-positive rates above project acceptance thresholds and must not be treated as audit-touched. Audit sessions applying the empirical-fortification branch — and audit-cross-reference workflows in routine QA-census sessions per the post-S-0156 seed-graph census shape — read the `evidence` column as the gold signal that the edge has been reasoned about; absent `evidence`, the edge is parametric and the audit treats it as such. Migration annotations ([`engine/operations/migration-discipline.md`](../operations/migration-discipline.md) Layer 2.5) reflect this signal by populating `evidence` at the time of any audit-directed correction; downstream cold-review of audit findings verifies that all graph mutations derive from edges with `evidence` text present.
