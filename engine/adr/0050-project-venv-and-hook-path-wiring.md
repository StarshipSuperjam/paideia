...
- [ADR 0091](0091-engine-memory-substrate-sqlite-fts5.md) — S-0188 substrate replacement. The `mempalace` CLI no longer needs to be on PATH; `chromadb` and `mempalace` Python pins are removed from `pyproject.toml` at S-0193. The broader venv-isolation + PATH-wiring discipline survives; `engine/memory/` runs on stdlib `sqlite3` (no new pinned dep). The `scrub_env.sh` PATH-prepend continues to apply to all subprocesses including the new MCP server.
