...
**5. `apply_migration.py:302` is allowlisted.** The migration-version emission `strftime("%Y%m%d%H%M%S")` is the format Supabase's `schema_migrations.version` column expects; touching it would require a coordinated migration of historical version values. Per Issue #33's explicit scope statement, the format is allowlisted as a legacy supabase contract. The validator's `timestamp_helper_bypass` check excludes `apply_migration.py` by path. An inline comment at the emit site cross-references this ADR.
