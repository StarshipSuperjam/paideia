...
- New: engine/tools/mempalace_set_sync_threshold.py (retired at S-0193 per [ADR 0091](0091-engine-memory-substrate-sqlite-fts5.md) — HNSW sync_threshold concerns deleted with the chromadb substrate; the engine_memory SQLite + FTS5 substrate has no equivalent persist-tuning surface). Original delivery: CLI wrapper with `--threshold N` / `--palace-path PATH` / `--verify-only` / `--backup-dir PATH` / `--no-backup` / `--skip-probe` flags; exit codes 0/1/2/3/4/5/6 (success / divergent-verify-only / pre-flight-refused / chromadb-modify-error / post-switch-verification-failed / generic-pre-flight-error / probe-finding-but-modify-succeeded).
