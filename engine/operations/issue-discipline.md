...
- **Audit-trail line, only when `health-check-finding`-labeled issues exist:** `[session-start] Active health-check audits: N (M open recommendations).` followed by one indented line per audit-report path. Added at S-0088 per [Issue #47](https://github.com/StarshipSuperjam/paideia/issues/47). The line preserves the audit → recommendation lineage that the undifferentiated FYI count would otherwise erase: future audits at the cadence cycle spawn new `health-check-finding`-labeled Issues; this surface lets the reader see which audits are still open without digging through the backlog. Audit cross-references in issue bodies are detected via the `engine/docs/audits/<file>.md` URL/relative-path pattern; an issue tagged `health-check-finding` without a detectable audit ref still counts toward `M` (recommendation) but not toward `N` (audit lineage), so `M >= N` always (`M > 0, N == 0` is legitimate when the body cites no audit).
