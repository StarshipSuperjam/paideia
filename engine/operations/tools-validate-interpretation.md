...
Per [ADR 0063](../adr/0063-validator-tiered-runtime-targets-and-regression-soft-warn.md) (S-0126; four-phase model from S-0127 Issue #90; HISTORY_FILE pinning from S-0205 Issue #150; structural-phase target bumped 500→700ms at S-0206 third-fire Path C closure). Fires when any one of the four validator phases (`duration_structural_ms`, `duration_health_probe_ms`, `duration_graph_audit_ms`, `duration_total_ms`) exceeds 1.5× its tiered target (700ms / 5000ms / 5000ms / 11000ms) across the last 3 consecutive runs in `engine/tools/validate-history.jsonl` (the current run participates in the rolling window). Pre-S-0126 entries that carry only `duration_ms` and pre-S-0127 entries that carry the three-field schema are skipped (insufficient per-phase fields).
...
- `edge_contestability_unguarded_high_confidence` (S-0208 per [Issue #152](https://github.com/StarshipSuperjam/paideia/issues/152) + [ADR 0097](../../product/adr/0097-tier-a-cluster-1-contestability-substrate.md) Consequences-deliverable) — edge with `expert_confidence >= 0.7` (high band per ADR 0097 enum convention low=0-0.4 / medium=0.4-0.7 / high=0.7-1.0) AND `counterexamples = []` AND `provenance.rationale` null or empty. The atomic-contestability failure mode from synthesis.md §Cluster 1 top-level finding 3 — high confidence without recorded exceptions or justification is structurally unguarded. Disposition: SQA review per Cluster 7 SQA predictive-validity pipeline — either populate `counterexamples` + `provenance.rationale` to justify the high confidence (preserving the claim with explicit guards) or downgrade `expert_confidence` to medium/low (acknowledging the absence of justification). Post-Cluster-2 (migration 0130 at S-0208) expected firing pattern: ~17 fires against the `historical_influence` edges that retained `expert_confidence = 1.0` from prior production-audit follow-ups; the 516 retyped `soft_prerequisite` edges similarly fire because their `expert_confidence` was preserved at 1.0 from prior production-audit work — Cluster 7 SQA validation is the natural author of confidence downgrades or rationale populations as learner-trace evidence accumulates.
- `edge_provisional_hard_prerequisite` (S-0208 per [ADR 0098](../../product/adr/0098-tier-a-cluster-2-edge-type-taxonomy-and-three-relation-layering.md) Consequences-deliverable) — edge typed `edge_type = 'hard_prerequisite'` AND `expert_confidence < 0.4` (low band per ADR 0097 enum convention). Threshold rationale per ADR 0098 premise 6 + paper_1:L162 (experts overstate necessity; hard-typed + low-confidence is the structurally suspect combination). Post-Cluster-2 expected firing pattern: 0 (all 516 retyped to `soft_prerequisite` per ADR 0098 premise 2). Future fires surface manual hard-typings without SQA-validated expert confidence; disposition: SQA review per Cluster 7 pipeline — either upgrade `expert_confidence` to medium/high with documented evidence (preserving the hard-prereq claim with justified confidence) or retype the edge to `soft_prerequisite` (acknowledging the absence of justification for the strong claim).
...
- **Validator runtime budgets** — per [ADR 0063](../adr/0063-validator-tiered-runtime-targets-and-regression-soft-warn.md) the tiered targets are: structural phase < 700ms (in-process file/regex checks; no DB, no subprocess; bumped from 500ms at S-0206 third-fire Path C closure reflecting corpus growth 85→92 ADRs + 7→10 structural-phase functions), health-probe phase < 5s (external-subprocess probes: repo state via `probe_repo.py`, session-dir via `probe_session_dir.py`, GitHub Issues via `gh issue list`, lockfile freshness via `uv lock --check`, Dependabot PRs via `gh pr list`), graph audit phase < 5s (live-DB consultation per [ADR 0016](../adr/0016-graph-construction-needs-live-validation.md)), total runtime < 11s. The `validator_runtime_phase_regression` soft-warn fires when any phase exceeds 1.5× its target across 3 consecutive runs. Investigate the phase's hot path on the soft-warn, correct the phase boundary if a concern is misclassified, or — with evidence the steady-state has shifted legitimately — adjust the target in `VALIDATOR_PHASE_TARGETS_MS`.
