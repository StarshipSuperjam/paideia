...
0a. **Boot-freshness gate** (per [ADR 0082](../adr/0082-routine-boot-freshness-and-concurrency-defense.md)). Run `engine/tools/routine_boot_freshness.py`. Mechanically fast-forwards the worktree to `origin/main` before any shared-state read. Exit 0 → proceed. Exit 2 (HEAD diverged) → write HANDOFF "routine boot refused: HEAD diverged from origin/main" with `**Disposition:** out-of-scope` → exit 0 without claiming. See "Concurrency control" below for the why.

0b. **Concurrency lock** (per ADR 0082). Run `engine/tools/routine_lock.py acquire`. Exit 0 → proceed. Exit 1 (lock held by another fresh process) → log "another routine in progress, exiting cleanly" → exit 0 without claiming. The lock is released at step 11.
...
5.5. **engine_memory boot query** (per ADR 0091, S-0192). Invoke `python3 -m engine.memory.boot_surface`. The orchestrator resolves the work-item phrase from `auto_target.json`'s active task `name`, runs three formulations (literal / conceptual / adjacent) through FTS5 + BM25 + recency + tag-class-boost retrieval against `engine/.memory/engine_memory.sqlite3`, deduplicates and ranks, writes an idempotent `## Prior context (engine memory)` section into `engine/session/current_plan.md`, and writes one `query_log` row per formulation. Mechanically backstopped by `engine_memory_boot_query_skipped` soft-warn (per ADR 0091) — fires at shutdown if no `engine_memory_search` call landed.
5.6. **engine_memory diary read** (per ADR 0091, S-0192). Call `engine_memory_diary_read agent_name="claude" last_n=3`. Mechanically backstopped by `engine_memory_diary_read_skipped` soft-warn.
...
8. **Claim slot.** Standard eager-claim ritual: bump `register_state.json`, write `engine/session/current.json`, mark task `in_progress` in target file, commit, FF main, push. Routine-specific `current.json` overrides on top of the [`session-build-lifecycle.md`](session-build-lifecycle.md) template: set `mode` to `"routine"` (NOT the template's `"interactive"` default — required from S-0048 per [ADR 0051](../adr/0051-routine-mode-and-engine-loop.md), hard-fail-enforced on the close commit), set `working_on` to `"Routine task <task_id>: <task_name>"`, set `declared_scope` from the task's `scope_lock.allowed_paths` plus a one-line summary, and set `task_id` to the active task's id and `target_id` to the target file's `target_id`. The `task_id` and `target_id` fields are load-bearing — `routine_lifecycle_push.py` deliverable-mode and `check_routine_scope.py --plan` detect routine-mode via `current.json.task_id` and refuse / skip if it is absent; the S-0159 archive confirms the canonical routine `current.json` shape carries both.

   **Push-rejection branch** (per ADR 0082). If `git push origin main` is rejected, run `engine/tools/routine_eager_claim_recovery.py`. Exit 0 → recovery complete (HEAD reset to origin/main); release lock, exit cleanly without re-claiming. Exit 2 → ambiguous state; write HANDOFF "eager-claim race recovery refused: ambiguous state" with `**Disposition:** out-of-scope`, release lock, exit 0.
...
10. **Verify completion.** After work commits clean: re-run task criteria via `check_target.py --task-id <id>`. All pass → set the task's `status` to `complete` in `auto_target.json` and **stage it — do NOT create a standalone commit**; the status flip rides with the close commit at step 11 (`verify_close_shape` permits `auto_target.json` via the operational allowlist). Any fail → set `status` to `blocked` with `blocked_reason: <reason>`, stage it (same — rides with close), write a HANDOFF entry. A standalone `chore(session): mark <task> complete` commit matches none of `routine_lifecycle_push.py`'s three modes and cannot be pushed; left local it makes the close push 2-ahead → REFUSED (Issue #122).
11. **Shutdown.** Run the standard shutdown sequence per [`session-shutdown-sequence.md`](session-shutdown-sequence.md). Issues created during the session count into `outcome_summary` for shutdown review. **Last action**: `engine/tools/routine_lock.py release` (per ADR 0082; releases the step-0b lock for the next routine fire).

   **Issue #27 root cause (Phase 5 silent diary-skip across 12/16 routine sessions).** The pre-S-0078 routine-mode-lifecycle SKILL enumerated a SUBSET of canonical shutdown steps that omitted the diary write and the pushback/lesson capture asks; routine sessions followed the enumeration in practice. Closed at S-0078 by (a) explicitly enumerating diary write + pushback/lesson capture in the routine-mode-lifecycle SKILL step 11 and (b) mechanizing the diary-write check as `engine_memory_diary_write_skipped` hard-fail in `validate.py --final-check`. The shutdown sequence's engine_memory activity rollup (`scan_engine_memory_activity.py` before the audit pass) writes the `engine_memory_activity` field to `current.json` so the audit can see the per-tool call counts.

### Concurrency control (per ADR 0082)
...
Routine sessions never hard-fail on engine_memory availability. When a routine session closes without a `engine_memory_diary_write` call AND without an `engine_memory_unavailable_acknowledged:` token in `outcome_summary`, the validator emits a soft-warn `engine_memory_diary_write_skipped_routine` (LOUD body) and appends an entry to [`engine/session/diary_pending_index.json`](../session/diary_pending_index.json). The session closes cleanly; the diary entry is deferred, not lost.
...
1. **Verify the substrate is reachable.** Run `python3 -c "from engine.memory.connection import healthcheck; healthcheck()"` — successful exit means the SQLite file is readable and the schema is intact. The engine_memory substrate is a local file with no MCP-server-lifecycle dependency; "reconnect" cases are rare (filesystem error or permission change), not the routine occurrence MemPalace had.
...
4. **For each pending entry**, in order:
   - Read the archived session at `archive_path` (e.g. `engine/session/archive/S-0123.json`). Focus on `outcome_summary`, `scope_delivery`, `work_done` (if present), and `engine_memory_activity`. These are sufficient to reconstruct what the session would have written if MCP had been live.
   ...
   - Call `engine_memory_diary_write` with the original session's identifier in the entry body (the AAAK `SESSION:` field) but with `agent_name="claude"` and the reconstructed entry as the content. The write lands in the standard `wing_claude` diary.
   - Once the write succeeds, remove that entry from `pending` in `diary_pending_index.json`.

5. **Commit the index update** with a `chore(session):` or `fix(engine-memory):` commit message that names which sessions' diaries were recovered. Push as part of the recovery session's normal lifecycle.

**Failure modes:**

- *Substrate unreachable at recovery time.* Don't proceed with reconstruction; investigate the filesystem error and verify substrate alive (`python3 -c "from engine.memory.connection import healthcheck; healthcheck()"`), and retry. The index entries are non-volatile until you remove them.
...
- [ADR 0082](../adr/0082-routine-boot-freshness-and-concurrency-defense.md) — three-layer routine-boot defense (freshness/lock/recovery) the wrapper extends as a fourth layer.
