...
**Bifurcated pushback storage is acceptable** (clarification per S-0098 adjudication of S-0097 audit Fit-1). In addition to the `pushback`-tag-on-drawer convention above, the project also exercises a content-prefix convention: `[pushback]` as the leading content-prefix in drawers stored in `paideia/problems` room (S-0005, S-0048, S-0071 are the canonical instances). Both conventions are functionally equivalent for retrieval — semantic search on pushback-related queries surfaces both shapes, and the audit's freshness probe at S-0097 returned strong recall scores (0.535–0.819 across representative-term queries) covering content from both storage locations. The script's `audit_mempalace()` count of `pushback`-tagged drawers reports zero when only the content-prefix shape is exercised; the count is mechanically correct (the tag is absent) but functionally misleading. Extending the script's count to recognize both forms is tracked as an Issue per the S-0098 adjudication of S-0097 Fit-2. In the meantime, capture pushback content via either convention and trust semantic search to find both.
