...
**Token tightening (S-0089).** The token only downgrades the hard-fail IF `mempalace status` actually fails at close-time. If the substrate is reachable, the token is INVALID — the AI claimed unavailable but reality contradicts — and a new hard-fail (`mempalace_diary_write_skipped_invalid_token`) blocks the close. Closes the S-0087 + S-0088 burial pattern: in both sessions the AI saw no `mcp__mempalace__*` tools in its deferred-tool list at boot and defaulted to the token, but the substrate was actually live (the S-0088 conversation's resume hook surfaced 30 mempalace tools that the boot hook hadn't). The single-session use of the (valid) token is itself investigation-worthy under the tightened contract — the validator's body for engine sessions carries a LOUD ⚠️ "DO NOT BURY THIS" header so the surface cannot get buried in the validator's stderr flood.
