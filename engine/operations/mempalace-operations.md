...
- **`mempalace_diary_write` returns `-32000 "Internal tool error"` on cold-start embedder load.** Diagnosed at S-0146 ([Paideia #114](https://github.com/StarshipSuperjam/paideia/issues/114) closed; upstream filed as [MemPalace/mempalace#1495](https://github.com/MemPalace/mempalace/issues/1495)). The first chromadb-write tool call after a fresh `mempalace-mcp` process start can take long enough to load the ONNX/CoreML embedder that the MCP client times out and returns `-32000` with no recoverable trace (mempalace logs to stderr only, which Claude Code does not capture). The handler at `mcp_server.py:1378-1452` is well-defended (try/except returns `{"success": False, "error": ...}`); `-32000` only escapes when the handler itself raises before the inner try-blocks engage — which the cold-load timeout produces because the call is killed client-side, not server-side. After the embedder loads server-side, every subsequent chromadb-write succeeds. **Diagnostic recipe** (the technique that closed [paideia#114](https://github.com/StarshipSuperjam/paideia/issues/114)): bypass the MCP RPC layer with `.venv/bin/python3 -c "from mempalace.mcp_server import tool_diary_write; print(tool_diary_write(agent_name='probe', entry='probe', topic='diagnostic'))"` — direct success means the failure is in the framework / timeout layer, not the handler. **Workaround:** the `mempalace_unavailable_acknowledged: <reason>` token in `outcome_summary` (per [ADR 0056](../adr/0056-mempalace-mechanical-adoption-checks.md)) downgrades the close-time diary-write hard-fail to a soft-warn; reflections preserved as drawers via `mempalace_add_drawer` to `wing_<agent>/diary` are functionally equivalent for retrieval. Worked at S-0144, S-0145. **Mitigation paths under upstream review:** optional `MEMPALACE_LOG_FILE` env var for stderr persistence and optional embedder warmup at MCP server start (both proposed in upstream Issue #1495 with patch sketches).
...
**Persist-threshold tuning** (S-0145 per [ADR 0079](../adr/0079-hnsw-sync-threshold-tuning.md), [Issue #113](https://github.com/StarshipSuperjam/paideia/issues/113)). Mempalace sets `hnsw:sync_threshold = 50_000` on collection creation via `_HNSW_BLOAT_GUARD` (upstream PR #344 anti-bloat tuning for bulk mines); chromadb's `_apply_batch` only writes `index_metadata.pickle` when `_num_log_records_since_last_persist >= sync_threshold`. For the project's 1–3-drawers-per-session MCP write pattern, 50_000 is rarely reached → metadata stays in process memory → process exit drops it → next session boot finds the never-flushed state (root cause of the S-0078 + S-0144 destructive rebuilds at ~66-session interval). [`engine/tools/mempalace_set_sync_threshold.py`](../tools/mempalace_set_sync_threshold.py) overrides to a low steady-state value (default 100) via chromadb's `collection.modify(configuration={"hnsw": {"sync_threshold": N}})` retrofit path — the configuration channel is authoritative in chromadb 1.5.x (the metadata channel is a creation-time advisory and does not reflect post-modify updates). Two wiring points:

- **Boot probe** (verify-only; runs from [`session-start.sh`](../tools/hooks/session-start.sh) after `probe_palace.py`). Read-only; never auto-applies. Surfaces divergence as a LOUD attention block naming the corrective command. Modification ALWAYS requires explicit `--backup-dir PATH` per the user-directed safeguard.
- **Post-rebuild** (forward-pointer from [`mempalace_rebuild_hnsw.py`](../tools/mempalace_rebuild_hnsw.py)). After a successful scratch-then-swap rebuild, the rebuild tool's stderr names the threshold-set command + backup-dir convention. The post-swap operation is its own backup-required action, not auto-invoked.

**Hybrid posture across the lifecycle.** Bulk rebuilds inherit mempalace's `_HNSW_BLOAT_GUARD` 50_000 default on the scratch palace (the anti-bloat protection PR #344 designed). Steady-state writes against the live palace use threshold=100 (sealed recurrence vector). The hybrid honors both failure modes — never-flushed-metadata AND `link_lists.bin` sparse-file bloat — without trading one for the other.

**Threshold-value discipline.** ADR 0079 records the threshold-value deliberation honestly: 100 is defensible against threshold=5 (would trigger ~3,355 persistDirty() calls during a 16K-record rebuild, matching the bloat-feedback-loop pattern PR #344 documents) and against threshold=1000 (chromadb's untuned default; persists every ~300 sessions for low-volume workloads — insufficient improvement). Tunable via `--threshold N`; chromadb's `hnsw_params.py:22` validator floors at `int > 2`.

**Upstream filing.** New Issue on MemPalace/mempalace frames the low-volume-MCP gap and proposes a `--hnsw-sync-threshold N` flag on `mempalace init` / `mempalace repair --mode from-sqlite`. If upstream lands the API, the project-side wrapper retires as vestigial — same disposition as [`mempalace_rebuild_hnsw.py`](../tools/mempalace_rebuild_hnsw.py) vis-à-vis upstream Issue #1394. **S-0153 corroborating datapoint.** Sarah Novotny's [MemPalace/mempalace#1489](https://github.com/MemPalace/mempalace/issues/1489) comment (2026-05-13) confirmed the never-flushed-metadata symptom reproduces after a 28K-drawer `mempalace repair --yes` bulk rebuild — not just for low-volume MCP writes. The hybrid posture above already handles both branches, so the workaround is unaffected; the comment also surfaced an observability vector (quarantine-directory accumulation under the palace root: `.drift-*` from never-flushed-metadata + `.corrupt-*` from harder failure shapes, both placed by upstream `quarantine_stale_hnsw` per MemPalace/mempalace#1322 + #1342, compounding silently as each open replaces a bad segment with a fresh-empty placeholder the next open also quarantines). [`probe_palace.py`](../tools/probe_palace.py) now emits `[probe-palace] quarantine: drift=N corrupt=N`; [`validate.py`](../tools/validate.py) surfaces the `mempalace_quarantine_accumulation` soft-warn when total ≥ 1 (snapshot count; cross-session rate is the lifecycle signal per [ADR 0042](../adr/0042-soft-warn-lifecycle-archive-canon.md)).

**S-0145 first-exercise.** Live palace converged from `sync_threshold=50_000` to `100` via `mempalace_set_sync_threshold.py --threshold 100 --backup-dir ~/.mempalace/palace.S-0145-pre-threshold-switch`. Verified post-switch via direct chromadb inspection: `configuration.hnsw.sync_threshold=100` on both `mempalace_drawers` (16,792 records) + `mempalace_closets` (51 records); `space=cosine` preserved. Backup retained at `~/.mempalace/palace.S-0145-pre-threshold-switch` as the rollback point.

**Pollution control** (S-0186 per [ADR 0090](../adr/0090-phase-6-recall-substrate-decision.md) commitment 2b, [Issue #131](https://github.com/StarshipSuperjam/paideia/issues/131)). Upstream `mempalace/hooks_cli.py:649-650` hardcodes `--wing sessions` on the `_ingest_transcript` subprocess that every Stop / PreCompact hook fire invokes. Result: the `sessions` wing accumulates terminal-output capture across `room IN ('technical','planning','architecture','general','problems')` at a rate of ~50-100 drawers/session, displacing curated content in `mempalace_search` BM25 fallback when vector search is degraded. The S-0184 freshness probe found this pollution dominating top-N results across three representative queries. **Two layers of mitigation:**

- **One-shot historical drain** via [`engine/tools/prune_mempalace.py`](../tools/prune_mempalace.py) `--audit-sessions-pollution --apply --backup-dir <path>`. Wing-exact (`sessions`) + room-IN-noise-rooms classifier; curated rooms (`decisions`, `lessons`, `pushback`, `diary`, `operations`) in the same wing are preserved by room-whitelist exclusion. Dry-run by default; same `--backup-dir` gate as the other audit modes. The S-0186 first-exercise drained 21,893 pollution drawers from a 22,676-drawer palace (97% reduction) while preserving 105 `sessions/decisions` curated drawers.
- **Ongoing defensive prune** via [`engine/tools/mempalace_post_hook_prune.py`](../tools/mempalace_post_hook_prune.py) invoked from [`engine/tools/hooks/mempalace-hook-wrapper.sh`](../tools/hooks/mempalace-hook-wrapper.sh) on every probe-healthy hook completion. Bounded scope: same wing+room classifier as the audit mode, plus a `created_at > $HOOK_START_TS` filter so only drawers added by THIS hook fire are touched. Idempotent; advisory exit code (the hook wrapper does not block on prune errors). Logged as `post_prune_deleted=N` in `.claude/logs/mempalace-hook.log`.

**Retirement criterion.** Both layers retire when upstream [MemPalace/mempalace#1511](https://github.com/MemPalace/mempalace/pull/1511) (`MEMPALACE_WING` env-var override) lands and the hook wrapper wires the override to direct transcript ingest into `wing_paideia` (or equivalent) at source. At that point the pollution stops being generated and the defensive prune has no candidates to find; the audit-mode prune retires the same session. Per ADR 0090 commitment 3 (upstream coordination posture).
