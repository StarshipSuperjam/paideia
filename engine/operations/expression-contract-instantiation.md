...
Three layers compose. Each pattern carries Layer 1 plus at least one of Layer 2 or Layer 3. A pattern with side-effecting deployment (currently SQL/migrations only) additionally carries Layer 2.5.
...
- **Layer 2.5 — Empirical post-deployment verification** (per [ADR 0039](../adr/0039-universal-expression-contract-across-ai-authoring-patterns.md) Consequences amendment landed at S-0094; currently scoped to SQL/migrations only). Patterns whose authoring produces side-effects against an external system (the live DB, in the SQL/migrations case) carry an additional layer that queries that system after the artifact deploys, comparing actual state against contract-declared expectations. Catches the silent-skip class of bugs Layer 2 cannot see (the SQL claims `INSERT 28 rows`; the DB might have 27 due to FK violation rolled back inside `DO $$`, ON CONFLICT DO NOTHING, CHECK rejected and continued). Patterns without side-effecting deployment skip Layer 2.5 — Python tests run in-process, prose ships as text, TypeScript edge functions are deployed but their postconditions are checked at runtime by callers rather than at deploy time.
...
| SQL / migrations | `product/seed-graph/migrations/**/*.sql` | [`migration-discipline.md`](migration-discipline.md) | **Layer 2** (pre-commit): hand-rolled grep checks (transaction wrap, CASCADE on FKs targeting `users.id`, RLS-enable on `public.*` tables, CHECK constraint shape on enum-modeled TEXT columns) invoked by `validate.py`'s `validate_sql_gates()`; `sqlfluff lint --dialect postgres` when available. **Layer 2.5** (post-apply, pre-record; per [ADR 0039](../adr/0039-universal-expression-contract-across-ai-authoring-patterns.md) Consequences amendment landed at S-0094): `engine/tools/apply_migration.py` parses the inline `-- Postcondition-Assertions:` block from the contract header and runs each `<SELECT>::<int>` against the live DB after body apply; mismatches surface exit code 8 with all failures listed. See [`migration-discipline.md`](migration-discipline.md) "Layer 2.5". | Cold-review pass on `product/seed-graph/migrations/**/*.sql` diff at session shutdown; sub-agent reads migration-discipline.md and diff, reports premise-vs-implementation mismatches; for migrations whose Layer 2.5 verified, the prompt directs the reviewer to focus on premise drift Layer 2.5 cannot catch (e.g., structural claims like "all edges within-domain") | [ADR 0039](../adr/0039-universal-expression-contract-across-ai-authoring-patterns.md) (S-0035 + S-0094 amendments) |
