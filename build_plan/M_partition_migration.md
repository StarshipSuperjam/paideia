...
> Bridge session(s) between Phase 2 close (S-0023) and Phase 5 open. Executes the `git mv` per [ADR 0037](../engine/adr/0037-engine-product-wall-and-changelog-rename.md), updates every cross-reference in the tree, runs `tools/validate.py` clean. The first per-phase build session ([`P_1_sql_schema.md`](P_1_sql_schema.md)) opens against the migrated tree.

## Phase scope

The repo restructures into `engine/` and `product/` subtrees per the shape committed in [ADR 0037](../engine/adr/0037-engine-product-wall-and-changelog-rename.md). Phase 5 seed-graph authoring is the partition's first content load-test; the migration must close before Phase 5 opens so the seed graph lands at its partition-aware path from the first migration file.
...
The post-migration tree per [ADR 0037](../engine/adr/0037-engine-product-wall-and-changelog-rename.md):
...
- [`adr/0037-engine-product-wall-and-changelog-rename.md`](../engine/adr/0037-engine-product-wall-and-changelog-rename.md) — the partition contract.
...
- [`adr/README.md`](../product/adr/README.md) — for the ADR index that gets split across `engine/adr/README.md` and `product/adr/README.md`.
...
- [ADR 0037](../engine/adr/0037-engine-product-wall-and-changelog-rename.md) — the partition shape and the sequencing constraint.
- [ADR 0036](../engine/adr/0036-expression-contract-for-inward-documents.md) — the inward-voice contract; migrated documents preserve the contract.
- [ADR 0027](../product/adr/0027-rendering-policy-prompt-layer-contract.md) and [ADR 0028](../product/adr/0028-input-side-scope-structural-not-prompt.md) — the rendering policy ADRs that, per ADR 0037, file under `product/adr/` because their consumer-of-record is learner-facing.

## ADR partition criterion (to settle in-session)

Each ADR files into one of:

- **`engine/adr/`** — ADRs about how the AI build apparatus works. Examples likely to land here: [ADR 0016](../engine/adr/0016-graph-construction-needs-live-validation.md) (graph validation), [ADR 0022](../engine/adr/0022-periodic-project-health-checks.md) (health checks), [ADR 0036](../engine/adr/0036-expression-contract-for-inward-documents.md) (inward-doc voice), [ADR 0037](../engine/adr/0037-engine-product-wall-and-changelog-rename.md) itself.
- **`product/adr/`** — ADRs about Paideia as a product (pedagogy, learner model, curriculum, business). Examples: [ADR 0001](../product/adr/0001-pedagogical-edges-not-historical.md)–[ADR 0012](../product/adr/0012-freshman-defaults-autodidact-ceiling.md) (the strong working commitments), [ADR 0027](../product/adr/0027-rendering-policy-prompt-layer-contract.md) (rendering policy — about how the AI talks to learners), [ADR 0028](../product/adr/0028-input-side-scope-structural-not-prompt.md), [ADR 0029](../product/adr/0029-personal-financial-cost-ceiling.md), [ADR 0030](../product/adr/0030-confidence-level-evidentiary-mode-on-nodes.md), [ADR 0031](../product/adr/0031-erasure-mechanism-and-individual-only-regime.md), [ADR 0032](../product/adr/0032-personal-project-disposition.md) (Superseded), [ADR 0033](../product/adr/0033-mission-realignment-structured-guidance-for-self-learners.md), [ADR 0034](../product/adr/0034-discovery-planning-engagement-triad.md), [ADR 0035](../product/adr/0035-multi-platform-apple-expansion.md).
- **Mixed / judgment-call** — ADRs that touch both. The judgment in-session settles each: which subtree has the load-bearing reader, which subtree is the citation-of-record. Candidates worth particular attention: [ADR 0013](../product/adr/0013-mastery-verification-organic-escalation.md)–[ADR 0021](../product/adr/0021-node-deprecation-status-superseded-by.md) (architectural decisions about the schema and engine internals — likely engine, but [ADR 0014](../product/adr/0014-sonnet-teaches-opus-reviews.md) and [ADR 0017](../product/adr/0017-postgres-recursive-ctes-over-owl-rdf.md) sit at the boundary), [ADR 0023](../product/adr/0023-engagement-depth-aggregation.md)–[ADR 0026](../product/adr/0026-persistent-learner-storage-structural-not-substantive.md) (mastery / learner-storage decisions — likely product because they describe learner-state representation).
...
Mechanical/procedural tier: target 70%, cap 80%. The work is mostly `git mv` and find-and-replace across cross-references; the substantive judgment is the ADR partition. The migration ran across two sessions per the natural split below: S-0024 executed the structural moves and ADR partition (closed at 0 hard-fails, 15 `cross_reference_broken` soft-warns); S-0025 executed the cross-reference sweep across the migrated tree (closed at 0 hard-fails, 0 soft-warns).
...
- **CLAUDE.md auto-load behavior.** Claude Code's auto-load walks ancestor directories UP from cwd; subdirectory `CLAUDE.md` files load lazily only when files inside that subtree are touched. A naive `git mv CLAUDE.md engine/CLAUDE.md` would break the boot ceremony — the standing rules would not enter context at session start. **Resolution (settled at S-0024):** `CLAUDE.md` stays at root per ADR 0037's edge-case (c), the same logic the ADR applies to `.claude/` ("Claude Code expects it there"). Both are harness-expected hook-points; the wall is conceptual, not a physical-purity test. Internal links inside `CLAUDE.md` update from `docs/operations/...` to `engine/operations/...` as part of the cross-reference sweep. The previously-enumerated mitigations (root symlink, thin pointer, `cd engine/` at boot, root pointer with `@engine/CLAUDE.md` import) are obviated — staying at root costs nothing and avoids the auto-load ceremony entirely.
