...
| [`M_partition_migration.md`](M_partition_migration.md) | Bridge | Engine / product folder migration per [ADR 0037](../adr/0037-engine-product-wall-and-changelog-rename.md) | `engine/` and `product/` subtrees populated by `git mv`; cross-references updated; `engine/tools/validate.py` clean. | First post-S-0023; one or two sessions. | Mechanical (target 70%, cap 80%). |
...
- **`P_1`** (Phase 3): no `OQ-*` blockers as of S-0023; `OQ-PRIVACY-A` and `OQ-PRIVACY-B` were closed by [ADR 0031](../adr/0031-erasure-mechanism-and-individual-only-regime.md) before S-0023.
...
Per [ADR 0037](../adr/0037-engine-product-wall-and-changelog-rename.md):
