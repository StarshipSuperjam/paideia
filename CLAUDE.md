...
- **Mid-session context-pressure monitoring** (closed at S-0048 per [Issue #4](https://github.com/StarshipSuperjam/paideia/issues/4) as intractable) — the 60% / 70% / 80% budget percentages in the **Budget guidance** section above are AI posture, not machinery, and remain so by deliberate choice. Live transcript-token measurement is infrastructurally blocked: Claude Code does not expose the in-progress transcript path (or a session-id env var that would let a hook resolve the transcript) to hook subprocesses. The candidate of "boot-time backstop against the prior session's archived `transcript_token_pct`" was evaluated and rejected: prior-session telemetry is too noisy a proxy (different work shape, doc set, and prose density per session) and tiktoken estimates are imprecise even at shutdown — a backstop keyed on those numbers would either fire spuriously and erode trust or set its threshold high enough to be useless. **Operating policy**: the user manually monitors context load during routine review; any session that archived with `transcript_token_pct >= 0.70` gets a post-hoc review pass to verify outputs are meaningful work-product, not summarized or hallucinated content. The cost of the gap is captured by review, not by mechanism.
- **`transcript_token_pct` is cumulative-session-content, not live prompt pressure** (closed at S-0052 per [Issue #11](https://github.com/StarshipSuperjam/paideia/issues/11)). The field captured by [`engine/tools/scan_context_telemetry.py`](engine/tools/scan_context_telemetry.py) at shutdown is an upper-bound estimate of total transcript content over the session's lifetime — including content the harness has cached or compacted out of the active prompt — divided by the 1M Opus 4.7 window for cross-session "running too long / too short / high variance" comparison per [`engine/operations/health-check.md`](engine/operations/health-check.md) Session-load trend section. **Do not use it to decide in-session budget posture.** The metric can exceed 1.0 in long sessions (3 of 10 archives in the S-0052 audit window did — S-0044=1.11, S-0046=1.62, S-0047=1.70); a value above 1.0 is the tell that the metric is cumulative, not live. The S-0048 closure of [Issue #4](https://github.com/StarshipSuperjam/paideia/issues/4) covers the absent-live-pressure-signal direction; this bullet covers the opposite-direction failure mode (treating the cumulative field as live). Both gaps are now named explicitly.
