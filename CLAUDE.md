...
- **Routine-mode harness permission allowlist** is the executability layer that complements the scope-lock above (per S-0046, after a routine fire deadlocked at the very first un-allowlisted Bash invocation). Routine sessions run unattended — every Bash command they invoke must be pre-approved in [`.claude/settings.json`](.claude/settings.json) `permissions.allow` or the session deadlocks at the prompt. Adding a new Bash invocation to the routine procedure (Skill body, slash command body, criterion predicate that shells out, per-task scope_lock workflow) REQUIRES adding a paired allowlist entry in the same commit. Pre-allowlisted surface covers engine python tools, worktree-side git lifecycle, test/lint stack, GitHub Issues path, Supabase migration tooling, hook scripts, shell utilities, and the MemPalace CLI. Layer 1 doc: [`engine/operations/routine-mode-operations.md`](engine/operations/routine-mode-operations.md) "Harness permission allowlist".
