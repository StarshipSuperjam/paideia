...
**Where deferrals go (per [ADR 0048](engine/adr/0048-handoff-narrowing-and-github-issues-for-cross-session-deferrals.md)).** HANDOFF.md is reserved for *session-internal handoffs* — work the next session must pick up immediately because the closing session was halted at a sensible boundary, or because there's an in-flight recovery procedure, or because work-item ownership transfers atomically. Cross-session deferrals — bugs, tech-debt, cleanup, enhancements, health-check findings — route to GitHub Issues via `gh issue create` with appropriate labels (`bug`, `enhancement`, `tech-debt`, `cleanup`, `health-check-finding`, `upstream`, `priority:urgent`). The choice is governed by [`engine/operations/issue-discipline.md`](engine/operations/issue-discipline.md): if the next session must address this before its own scoped work, HANDOFF.md; otherwise, Issue. The default is Issue. Future sessions see the backlog count at every boot and can dedicate cleanup-batch sessions when the count crosses a threshold worth addressing.
